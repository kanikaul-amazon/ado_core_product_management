...
BEGIN
    ...
    IF (TG_OP = 'INSERT') THEN
        INSERT INTO producthistory (productid, action, newprice, newstock, modifiedby, actiondate)
        VALUES (
            NEW.productid,
            'INSERT',
            NEW.price,
    ...
    ELSIF (TG_OP = 'UPDATE') THEN
        IF (NEW.price <> OLD.price OR NEW.stockquantity <> OLD.stockquantity) THEN
            INSERT INTO producthistory (productid, action, oldprice, newprice, oldstock, newstock, modifiedby, actiondate)
            VALUES (
                NEW.productid,
                'UPDATE',
                OLD.price,
    ...
    ELSIF (TG_OP = 'DELETE') THEN
        INSERT INTO producthistory (productid, action, oldprice, oldstock, modifiedby, actiondate)
        VALUES (
            OLD.productid,
            'DELETE',
            OLD.price,
...
FOR EACH ROW EXECUTE FUNCTION product_history_trigger_function();
